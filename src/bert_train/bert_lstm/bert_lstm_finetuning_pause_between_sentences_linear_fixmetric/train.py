...
def evaluate(model, dataloader, device, loss_fn, num_labels=1):
    ...
    if num_labels == 1:
        ...
        # サイズが同じであることを確認
        assert len(all_preds) == len(all_labels) == len(means) == len(vars_), f"{len(all_preds)}, {len(all_labels)}, {len(means)}, {len(vars_)}"
        # ポーズを実数値へ逆標準化
...
class CustomModel(nn.Module):
    ...
    def __init__(self, model_name, tokenizer, num_labels=1, linear_pooling_type="none", layer_num=1):
    ...
    def feature(self, **inputs):
        ...
        lstm_output, (hidden, cell) = self.bilstm(last_hidden_states)
        ...
        if self.linear_pooling_type == "cls":
            ...
            # print(last_hidden_states.shape, first_sep_position, last_hidden_states[:, first_sep_position, :].shape, self.config.hidden_size, lstm_output.shape)
            feature = lstm_output[:, first_sep_position, :].view(-1, self.config.hidden_size * 2)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # [batch_size, hidden_size * 2] -> [batch_size, num_labels]
...
class EarlyStopping:
    ...
    def load_checkpoint(self, model):
        ...
        self.trace_func(f"ベストモデルを {self.counter} エポック時点の検証損失: {self.val_loss_min:.6f} で読み込みました。")
...
def run_training(cfg):
    ...
    model = CustomModel(
        cfg.model.name, tokenizer, cfg.model.num_labels, cfg.model.linear_pooling_type, cfg.model.layer_num
    )
    ...
    run_name = f"{str(cfg.dataset.pause_time_threshold_ms)}ms_{cfg.dataset.preprocess_type}_{cfg.model.num_labels}labels_linear_maxlen[{cfg.training.max_length_train}.{cfg.training.max_length_val}.{cfg.training.max_length_test}]"
