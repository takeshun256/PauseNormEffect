...
def evaluate(model, dataloader, device, loss_fn, num_labels=1):
    ...
    if num_labels == 1:
        ...
        # サイズが同じであることを確認
        assert (
            len(all_preds) == len(all_labels) == len(means) == len(vars_)
        ), f"{len(all_preds)}, {len(all_labels)}, {len(means)}, {len(vars_)}"
        # ポーズを実数値へ逆標準化
...
class CustomModel(nn.Module):
    ...
    def __init__(self, model_name, tokenizer, num_labels=1, linear_pooling_type="cls", layer_num=1):
