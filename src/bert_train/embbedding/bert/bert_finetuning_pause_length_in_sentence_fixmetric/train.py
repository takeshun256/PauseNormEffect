...
from transformers import BertConfig, BertForTokenClassification, BertJapaneseTokenizer, BertModel
    ...
    def __getitem__(self, index):
        ...
        if self.num_labels == 1:
            ...
            return {
                ...
                "group_id": torch.tensor(id_group, dtype=torch.long),
            }
        else:
            # 分類タスク
            return {
...
class CustomModel(nn.Module):
    ...
    def __init__(self, model_name, tokenizer, num_labels=1, linear_pooling_type="none", layer_num=1, num_groups=1):
        ...
        self.meanpooling = None  # 使う予定がないのでNone
    ...
    def forward(self, input_ids, attention_mask, **inputs):
        ...
        output = self.fc(feature_embedded)  # feature関数からの出力をlinear関数に通す
...
def run_training(cfg):
    ...
    run_name = f"{str(cfg.dataset.pause_time_threshold_ms)}ms_{cfg.dataset.preprocess_type}_{cfg.model.num_labels}labels_maxlen[{cfg.training.max_length_train}.{cfg.training.max_length_val}.{cfg.training.max_length_test}]_num_groups{num_groups}"
