    ...
    def __getitem__(self, index):
        ...
        if self.num_labels == 1:
            ...
            return {
                ...
                "group_ids": torch.tensor(id_group, dtype=torch.long),
            }
        else:
            # 分類タスク
            return {
...
def predict_and_evaluate(model, dataloader, device):
    ...
    if model.num_labels == 1:
        ...
        # meansとvarorがそれぞれ0,1の場合であることを確認
        assert np.all(means == 0) or np.all(means == 1), f"means: {means}"
        assert np.all(vars_ == 0) or np.all(vars_ == 1), f"vars_: {vars_}"
        # ポーズを実測値へ逆標準化
